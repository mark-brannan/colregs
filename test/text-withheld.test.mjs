...
// test/data.test.mjs over exactly that corpus (the en-US one, the only one
// with text). The assertions there that
...
const corpus = JSON.parse(readFileSync(new URL('../data/text/intl/2016/en-US.uscg.json', here)))
const schema = JSON.parse(readFileSync(new URL('../schema/corpus.schema.json', here)))

const withheld = {
  ...corpus,
  paragraphs: Object.fromEntries(Object.entries(corpus.paragraphs).map(([path, p]) => {
    const { text, ...structure } = p
    return [path, { ...structure, text_status: 'withheld', withheld_reason: 'stripped for the ADR 0010 invariant' }]
  })),
}

test('ADR 0010: a corpus with every text withheld validates against corpus.schema.json', () => {
...
test('ADR 0010: fixtures, integrity checks and the reference evaluator stay green with every text stripped', () => {
  ...
  const file = join(dir, 'en-US.uscg.json')
  ...
  const run = spawnSync(process.execPath, ['--test', '--test-reporter=tap', new URL('data.test.mjs', here).pathname], {
    encoding: 'utf8',
    env: { ...env, COLREGS_CORPUS_JSON: file },
  })
  rmSync(dir, { recursive: true, force: true })
  const summary = run.stdout.split('\n').filter((l) => /^not ok|^# (pass|fail)/.test(l)).join('\n')
  assert.equal(run.status, 0, `data.test.mjs is not green over a text-stripped corpus:\n${summary}\n${run.stderr.slice(-2000)}`)
