...
// PROSE_BUDGET_SKIP=1 to skip loudly on a machine without one.
...
const skip = !engine && process.env.PROSE_BUDGET_SKIP === '1' && 'PROSE_BUDGET_SKIP=1: SKIPPING prose budgets, no prose-budget engine found'

test('docs: prose budgets hold (prose-budget --tree --require-config)', { skip }, () => {
  assert.ok(engine, 'prose-budget engine not found; looked at $PROSE_BUDGET, `prose-budget` on PATH and $HOME/.local/bin/prose-budget. Set PROSE_BUDGET_SKIP=1 to skip.')
