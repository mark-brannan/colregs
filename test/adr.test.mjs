...
import { ROOT, INDEX, adrFiles, claimOf, pad, parseIndex, promote, readIndex, slugOf } from '../scripts/adr.mjs'
...
test(
  ...
  { skip: base === null && !process.env.CI && 'origin/main not fetched; run `git fetch origin main`' },
  () => {
...
const fixture = (index, proposals, budgets = {}) => {
  ...
  writeFileSync(join(root, 'docs/budgets.json'), JSON.stringify({ lines: budgets }, null, 2) + '\n')
  for (const [name, text] of Object.entries(proposals)) writeFileSync(join(root, 'docs/proposals', name), text)
  for (const args of [
    ['init', '-q'],
    ['add', '-A'],
...
const budgetsAt = (root) => JSON.parse(readAt(root, 'docs/budgets.json')).lines
...
test('promote: an unnumbered proposal takes the next free number', () => {
  ...
  assert.equal(existsSync(join(root, 'docs/proposals/some-idea.md')), false, 'the proposal is moved, not copied')
  assert.equal(readAt(root, p.dest), '# ADR 0002 — Some idea\n\nBody.\n', 'the H1 gains the number and the body is untouched')
  assert.equal(readAt(root, INDEX), '- 0001 [First](0001-first.md)\n- 0002 [Some idea](0002-some-idea.md)\n')
  assert.deepEqual(budgetsAt(root), { 'docs/adr/0002-some-idea.md': 60 }, 'the budget key follows the file')
  assert.equal(
    spawnSync('git', ['status', '--porcelain'], { cwd: root, encoding: 'utf8' }).stdout.includes('??'),
    false,
    'the move is staged, so the follow-up commit sees a rename',
  )
})

test('promote: a proposal with no budget line gets one at the length it landed at', () => {
  // docs/proposals/ is outside the delta rule and docs/adr/ is inside it, so
  // an ADR that arrives unbudgeted fails the promotion's own CI on its whole
  // word count. The budget is the file's line count as written: the H1 line,
  // the blank, three body lines.
  const root = fixture('- 0001 [First](0001-first.md)\n', { 'some-idea.md': '# Some idea\n\nOne.\nTwo.\nThree.\n' })
  promote(['docs/proposals/some-idea.md'], root)
  assert.deepEqual(budgetsAt(root), { 'docs/adr/0002-some-idea.md': 5 })
})
...
test('promote: a claimed number fills its reserved line in place', () => {
  ...
  assert.equal(p.n, 2, 'the claim is honoured rather than the next free number, 4')
  assert.equal(
    readAt(root, INDEX),
    '- 0001 [First](0001-first.md)\n- 0002 [Part D signals](0002-part-d-signals.md)\n- 0003 [Third](0003-third.md)\n',
    'the reserved line is replaced where it sits, not appended below 0003',
  )
  assert.equal(readAt(root, 'docs/adr/0002-part-d-signals.md').split('\n', 1)[0], '# ADR 0002 — Part D signals')
})

test('promote: a number that is not reserved is refused', () => {
  const root = fixture('- 0001 [First](0001-first.md)\n', { '0009-jumped-the-queue.md': '# Jumped the queue\n' })
  assert.throws(() => promote(['docs/proposals/0009-jumped-the-queue.md'], root), /ADR 0009 is not reserved/)
  assert.equal(existsSync(join(root, 'docs/proposals/0009-jumped-the-queue.md')), true, 'the refusal leaves the tree alone')
})

test('promote: a proposal with no H1 is refused', () => {
  const root = fixture('- 0001 [First](0001-first.md)\n', { 'no-title.md': 'Body with no heading.\n' })
  assert.throws(() => promote(['docs/proposals/no-title.md'], root), /no H1 on the first line/)
})
...
test('promote: promoting an already-moved proposal is a no-op', () => {
  ...
  assert.deepEqual(promote(['docs/proposals/some-idea.md'], root), [], 'a second label event promotes nothing')
  assert.equal(readAt(root, INDEX), before, 'and appends no second line for it')
})

test('promote: a path that is neither a proposal nor a landed ADR is refused', () => {
  // The idempotency skip is for a proposal already moved, and only that. A
  // typo on the command line must not come back as "nothing to promote".
  const root = fixture('- 0001 [First](0001-first.md)\n', { 'some-idea.md': '# Some idea\n' })
  assert.throws(() => promote(['docs/proposals/some-ideas.md'], root), /some-ideas\.md: not found/)
  assert.equal(existsSync(join(root, 'docs/proposals/some-idea.md')), true, 'and the real one is untouched')
})

test('promote: the same path named twice is one promotion', () => {
  // The workflow's diff cannot repeat a path; a hand-typed command can, and
  // the second git mv would fail after the first had already moved the file.
  const root = fixture('- 0001 [First](0001-first.md)\n', { 'some-idea.md': '# Some idea\n' })
  const promoted = promote(['docs/proposals/some-idea.md', 'docs/proposals/some-idea.md'], root)
  assert.deepEqual(
    promoted.map((p) => p.n),
    [2],
  )
...
test('promote: several proposals in one merge take consecutive numbers', () => {
  ...
  const promoted = promote(['docs/proposals/a.md', 'docs/proposals/0002-b.md', 'docs/proposals/c.md'], root)
...
test('promote: a bad claim mid-batch moves nothing at all', () => {
  ...
  assert.throws(() => promote(['docs/proposals/a.md', 'docs/proposals/0009-jumped.md'], root), /ADR 0009 is not reserved/)

  assert.equal(existsSync(join(root, 'docs/proposals/a.md')), true, 'the earlier proposal is not moved out from under the refusal')
  assert.equal(existsSync(join(root, 'docs/adr/0002-a.md')), false, 'and nothing lands in docs/adr without a register line')
  assert.equal(readAt(root, INDEX), '- 0001 [First](0001-first.md)\n')
  assert.deepEqual(
