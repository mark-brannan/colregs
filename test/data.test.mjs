...
const corpusDoc = (paragraphs, over = {}) => ({
  id: 'eu/cevni.fr.unece', jurisdiction: 'eu/cevni', language: 'fr', source_id: 'unece', tier: 'national',
  normalization: 'NFC', amendment_state: { amended_through: 'CEVNI Rev.6', in_force: '2021-01-01' },
...
// --- REQ-LANG-10: a corpus that carries text declares the amendment state
// its source reflects; only an empty stub may leave it undeclared ---------
test('corpus schema: amendment_state may be null only on an empty corpus', () => {
  const validate = new Ajv2020({ allErrors: true, strict: true }).compile(loadSchema('corpus.schema.json'))
  const verbatim = { rule_title: 'Overtaking', text: 'Every vessel overtaking any other shall keep out of the way.' }
  assert.ok(validate(corpusDoc({}, { amendment_state: null })), 'an empty stub may be undeclared')
  assert.ok(!validate(corpusDoc({ 6: verbatim }, { amendment_state: null })), 'text without a declared state must be rejected')
})
...
test('REQ-LANG-5: every corpus paragraph and gap resolves to a skeleton path, and none is both', () => {
  for (const [id, c] of Object.entries(corpora)) {
    for (const path of Object.keys(c.paragraphs)) {
      ...
      assert.equal(rules.paragraphs[path].jurisdiction, c.jurisdiction, `${id}: ${path} belongs to another jurisdiction`)
    }
...
test('ADR 0013: corpus id, file path and data/corpora.json agree with the metadata inside each corpus', () => {
  ...
  for (const [id, e] of Object.entries(corporaIndex.corpora)) {
    ...
    assert.equal(id, `${c.jurisdiction}.${c.language}.${c.source_id}`, `${id}: id is not <jurisdiction>.<language>.<source_id>`)
    assert.equal(e.file, `text/${c.jurisdiction}/${c.language}.${c.source_id}.json`, `${id}: file path disagrees with metadata`)
    assert.deepEqual(
      { jurisdiction: e.jurisdiction, language: e.language, source_id: e.source_id, tier: e.tier, paragraphs: e.paragraphs },
      { jurisdiction: c.jurisdiction, language: c.language, source_id: c.source_id, tier: c.tier, paragraphs: Object.keys(c.paragraphs).length },
...
test('REQ-LANG-10: a corpus whose amendment state differs from the skeleton is visible, never silent', () => {
  for (const [id, c] of Object.entries(corpora)) {
    if (!c.amendment_state) continue
    const skeleton = rules.amendment_state[c.jurisdiction]
    assert.ok(skeleton, `${id}: skeleton declares no amendment state for ${c.jurisdiction}`)
    // Not asserting equality: a stale corpus is legitimate. Asserting that the
    // comparison is possible, which is the whole of what REQ-LANG-10 asks.
    assert.equal(typeof c.amendment_state.amended_through, 'string')
  }
})
