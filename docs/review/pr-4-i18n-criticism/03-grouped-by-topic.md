...
below is a verbatim, complete copy of its source section (including that
section's own sub-headings), extracted programmatically to guarantee no
words, headers, or formatting were dropped. This document only *groups*
...
Labels: **Reviewer G** = doc 01 (general i18n critique, Gemini). **Reviewer C** =
doc 02 (detailed ADR-0003 review, ChatGPT).
...
**Reviewer G** (§4, "Nautical Terminology & Centralized Concept Dictionary"):
...
(No corresponding section in Reviewer C.)
...
**Reviewer C** (§15, "Unicode normalization deserves a decision"):
...
(No corresponding section in Reviewer G.)
...
**Reviewer G** (§8, "Fallback Chains & Packaging Strategy"):
...
(Argues the package *should* define a fallback chain.)

**Reviewer C** (§8, "Fallback being entirely the consumer's responsibility may be too strict"):
...
(Argues against the package prescribing a fallback chain, while still wanting more than nothing.)

**Reviewer C** (§9, "The design needs an explicit rule for mixing languages"):
...
(Related, no corresponding section in Reviewer G.)
...
**Reviewer C** (§2, ""Corpus per jurisdiction × language × source" is probably underspecified"):
...
**Reviewer C** (§3, "Paragraph paths are being treated as more stable than they really are"):
...
**Reviewer C** (§10, "You need to model amendments and consolidation sooner than later"):
...
**Reviewer C** (§16, "Translation shouldn't be assumed to preserve paragraph boundaries"):
...
**Reviewer C** (§12, "Source identity should probably be machine-readable, not just prose metadata"):
...
**Reviewer C** (§13, "Licensing needs to distinguish source rights from repository rights"):
...
**Reviewer C** ("A few smaller inconsistencies"):
...
**Reviewer C** ("One architectural change I'd seriously consider"):
...
**Reviewer G** ("Actionable Checklist for PR Review"):
...
**Reviewer C** ("My priority ranking"):
...
**Reviewer C** ("What I think is already very good" + "Bottom line"):
