...
- **Q-51** — **What arms 13(d)'s latch?** *A:* 13(b)'s deeming, at the first
  ...
  overtaking geometry with no risk of collision. Reversible: four entries
  (`13a`, `13b-*`, `13d`) and their fixtures. It stops being free once a
  published `colregs-engine` release's output is relied on, since this changes
  the answer for "overtaking geometry, clear, no risk" — not before.
- **Q-52** — **What does 13(d)'s latch forbid?** *Narrow:* reclassification to
  *crossing* only, as the paragraph says, leaving head-on to Rule 14 on the
  ...
  no case found on overtaking becoming head-on.
