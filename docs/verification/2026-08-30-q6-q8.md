...
**Verdict: REFUTED.** Of 90 paths, **15 are the dangerous case** — identical
path spelling, materially different denoted text — plus one structural
path-mismatch, one whole-rule clean absence, and two clean intl-only
absences. Roughly 71 of 90 are citation-compatible.
...
| 21(a) | Y | Y | SAME* | Repo's own text omits a "<12m centerline" clause both true IMO and CFR text carry — a repo transcription gap, not a CFR divergence. |
| 21(b) | Y | Y | SAME* | Same repo-gap pattern. |
...
| 23(b) | Y | Y | SAME* | CFR/true IMO text both carry "where it can best be seen"; repo's text alone omits it (repo gap). |
...
| 29(a), 29(a)(i–iii), 29(b) | Y | Y | SAME | Trivial wording only (repo's 29(b) has an extra "similar" not in CFR or true IMO text — likely a repo transcription quirk, not CFR-side). |
...
3. **"Prominent community plugins are permissively licensed" — REFUTED as a
   general statement.** Most of the highest-download `@signalk/*` packages
   (`freeboard-sk`, `signalk-to-nmea0183`, `course-provider`, …) are bundled
   dependencies of signalk-server itself, so their download counts measure
   server installs, not independent adoption — and those bundled ones are
   in fact Apache-2.0 or MIT. But among independently-installed plugins in
   the same download-count range, at least one prominent example is
   **AGPL-3.0-or-later**: `signalk-noaa-space-weather`
   (`github.com/mark-brannan/signalk-noaa-space-weather/blob/main/LICENSE`,
   confirmed by the LICENSE file header), at 4,351 weekly downloads —
   comparable to several of the Apache/MIT/ISC-licensed independents also
   checked (`signalk-to-nmea2000`, ISC; `signalk-n2kais-to-nmea0183`, ISC;
   `@signalk/udp-nmea-plugin`, Apache-2.0). One further plugin,
   `sk-ais-status-plugin`, claims `"Apache-20"` in package.json (not a real
   SPDX identifier) but carries no LICENSE file in its repo — unverifiable,
   not confirmed permissive either way.

**Bearing on ADR 0004**: the two premises the "every layer it consumes has
to stay permissive" argument actually needs — signalk-server's own license,
and in-process loading — both check out. The broader "the ecosystem is
uniformly permissive" framing does not; it should be narrowed to the
specific host (signalk-server) and specific plugins the switching plugin
will actually load, not asserted as an ecosystem-wide property.
