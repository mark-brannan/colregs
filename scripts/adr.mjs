...
export const nextNumber = (entries) =>
  Math.max(0, ...entries.map((e) => e.n)) + 1;
...
export const slugOf = (path) =>
  basename(path, ".md").replace(/^(?:\d{4}-\d{2}-\d{2}|\d{4})-/, "");
...
function rekeyBudget(root, from, to) {
  ...
  if (text.includes(`"${from}"`))
    writeFileSync(join(root, BUDGETS), text.replace(`"${from}"`, `"${to}"`));
}
...
export function promote(paths, root = ROOT) {
  ...
  for (const path of pending) {
    ...
    if (claim !== null && !held) {
      throw new Error(
        `${path}: ADR ${pad(claim)} is not reserved in ${INDEX}; drop the number and let the next free one be allocated`,
      );
    }
    ...
    if (!text.startsWith("# "))
      throw new Error(
        `${path}: no H1 on the first line; a proposal opens with its title`,
      );
    const nl = text.indexOf("\n");
    const title = (nl === -1 ? text : text.slice(0, nl)).replace(
      /^# (?:ADR \d{4} — )?/,
      "",
    );
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (cmd !== "promote" || paths.length === 0) {
    console.error(
      "usage: node scripts/adr.mjs promote docs/proposals/<slug>.md ...",
    );
    process.exit(2);
  }
